...
export const MovimientoSchema = z.object({
  ...
  orden_venta_id:   z.number().int().positive().optional(),
  ...
  guia_despacho:    z.string().max(50).optional(),
  observaciones:    z.string().max(1000).optional(),
});

export class MovimientoService {

  async create(dto: z.infer<typeof MovimientoSchema>, userId: number) {
    const material = await queryOne<any>(
      'SELECT * FROM materiales WHERE id = ? AND activo = TRUE', [dto.material_id]
    );
    ...
    if (dto.tipo === 'salida') {
      const [stock] = await query<any>(
        `SELECT COALESCE(SUM(CASE WHEN tipo='entrada' THEN cantidad WHEN tipo='salida' THEN -cantidad ELSE cantidad END),0) AS disponible
  ...
  async findAll(q: any = {}) {
    ...
    if (q.tipo)        { conditions.push('m.tipo = ?');        params.push(q.tipo); }
    if (q.material_id) { conditions.push('m.material_id = ?'); params.push(q.material_id); }
    if (q.desde)       { conditions.push('m.fecha >= ?');      params.push(q.desde); }
    if (q.hasta)       { conditions.push('m.fecha <= ?');      params.push(q.hasta + ' 23:59:59'); }

    const where = conditions.join(' AND ');
    const [total] = await query<any>(`SELECT COUNT(*) AS n FROM movimientos m WHERE ${where}`, params);
    const data = await query<any>(
      `SELECT m.*, mat.nombre AS material_nombre, mat.unidad,
       ...
       LEFT JOIN proveedores p ON p.id = m.proveedor_id
       LEFT JOIN usuarios u ON u.id = m.creado_por
       WHERE ${where} ORDER BY m.fecha DESC LIMIT ? OFFSET ?`,
      [...params, limit, offset]
    );
  ...
  async getStock() {
    return query<any>('SELECT * FROM stock_actual ORDER BY nombre');
  }

  async getStockById(materialId: number) {
    return queryOne<any>('SELECT * FROM stock_actual WHERE id = ?', [materialId]);
  }
}
